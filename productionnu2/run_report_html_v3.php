	...
	for($G = 0 ; $G < count($sectionArray) ; $G++){
		...
		if($sectionArray[$G]['the_page'] != $the_page){	//--end and start new page
			...
			for($c = 0 ; $c < count($S->controls) ; $c++){
				...
				$cSupposedToBeHLine	= ($S->controls[$c]->height == 0)||($S->controls[$c]->height == '0');
				$cSupposedToBeVLine	= ($S->controls[$c]->width == 0)||($S->controls[$c]->width == '0');
				...
				$CC             = $S->controls[$c];
				if($S->controls[$c]->type == 'Graph'){
					$data       = str_replace('#NUSINGLEQUOTE#', "'", $data);
				}
				if(! $cSupposedToBeHLine && ! $cSupposedToBeVLine ){
					//normal box object
				...
				}else if( $cSupposedToBeHLine ){
					//horizontal line
					print "           <div class='$cName' style='top:$cTop"."px;height:$cHeight"."px$style;border-left-width:0px;border-right-width:0px;border-bottom-width:0px;'>$tags$data$endtags</div>\n";
				}else{
					//vertical line
					print "           <div class='$cName' style='top:$cTop"."px;height:$cHeight"."px$style;border-right-width:0px;'>$tags$data$endtags</div>\n";
				}
			}
			print "\n   </div>\n";
        }
